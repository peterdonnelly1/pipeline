...
#ENV PYTHONPATH /home/peter/git/

RUN adduser --disabled-password --gecos 'default classi user' user_1

RUN \
    --mount=type=cache,target=/var/cache/apt \
    apt-get update && apt-get install -y git python3 python3-pip openslide-tools wget git tree vim
...
RUN --mount=type=cache,target=/root/.cache/pip pip install -r requirements_2.txt
RUN --mount=type=cache,target=/root/.cache/pip pip install -r requirements_1.txt
