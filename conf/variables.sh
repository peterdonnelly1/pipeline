...
if [[ ${DATASET} == "stad" ]]; 
  ...
  if [[ ${INPUT_MODE} == "image" ]]
    then
      ...
      TILES_PER_IMAGE="1"                                                 # Training mode only. <450 for Moodus 128x128 tiles. (this parameter is automatically calculated in 'just_test mode')
      ...
      LEARNING_RATE=".001"
      FINAL_TEST_BATCH_SIZE=12                                          # number of tiles to test against optimum model after each run (rna mode doesn't need this because the entire batch can easily be accommodated)
...
# 200913 - Poor       ---> USE_SAME_SEED="True";  N_EPOCHS=200; N_SAMPLES=479; BATCH_SIZE="95"; PCT_TEST=.2; LEARNING_RATE=".0008"; HIDDEN_LAYER_NEURONS="200";  NN_DENSE_DROPOUT_1="0.2;  GENE_DATA_NORM="JUST_SCALE" (59%, 65%, 66%, 68%); overall 65%
    ...
    then                                                                  # Also works well  HIDDEN_LAYER_NEURONS="700"; NN_DENSE_DROPOUT_1="0.2" <<< TRY IT AGAIN
      ...
      BATCH_SIZE="8"                                                     #  number of samples in each "mini batch"
...
#      LONG_CLASS_NAMES="adenocarcimoa_-_diffuse_type  intestinal_adenocarcinoma_-_mucinous_type    intestinal_adenocarcinoma_-_tubular_type  stomach_adenocarcinoma_-_signet_ring"
    ...
    then                                                                 # also works well  HIDDEN_LAYER_NEURONS="700"; NN_DENSE_DROPOUT_1="0.2" <<< TRY IT AGAIN                                                                          # Also works well  HIDDEN_LAYER_NEURONS="250"; NN_DENSE_DROPOUT_1="0.2"  << BEST SO FAR?
      BATCH_SIZE="8"                                                     # number of samples in each "mini batch"
      FINAL_TEST_BATCH_SIZE=1                                          # number of tiles to test against optimum model after each run (rna mode doesn't need this because the entire batch can easily be accommodated)
      TILES_PER_IMAGE="1"   ########################################   # MUST BE THE SAME AS THE VALUE USED IN [[ ${INPUT_MODE} == "image" ]] above
      N_SAMPLES="469"                                                    # 469 rna-seq samples (474 cases); 229 ??? have both (a small number of cases have two rna-seq samples)
      N_EPOCHS=500
      PCT_TEST="0.15"                                                    # proportion of samples to be held out for testing
