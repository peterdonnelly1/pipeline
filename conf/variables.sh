...
if [[ ${DATASET} == "stad" ]]; 
  ...
  if [[ ${INPUT_MODE} == "image" ]]; 
    then
      ...
      SUPERGRID_SIZE=20                                                    # for test mode only: enables 'super-patches' that combinine multiple batches into a grid [test_mode (only). Minimum/default value=1; maximum value depends in TILES_PER_IMAGE
      TILE_SIZE="128"                                                     # 
      TILES_PER_IMAGE=                                                 # Training mode only (automatically calculated as SUPERGRID_SIZE^2 * BATCH_SIZE for test_mode)
      BATCH_SIZE="144"
