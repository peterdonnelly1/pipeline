...
ZOOM_OUT_MAGS="  1    16   32"                                 # image only. magnifications (compared to baseline magnification) to be used when selecting areas for tiling, chosen according to the probabilities contained in ZOOM_OUT_CHOICE_PROBABILITIES
ZOOM_OUT_PROB="  .9  .05  .05"                    # image only. Chosen for magnification according to these probabilities, which must add up to 1
...
if [[ ${DATASET} == "stad" ]]; 
  ...
  if [[ ${INPUT_MODE} == "image" ]]
    then
      ...
      TILE_SIZE="232"                                                    # min 32, max 232 for MOODUS 
