...
ANNOTATED_TILES="False"                                                                                     # LESS PREFERRED. USE SCATTERGRAM INSTEAD. show annotated tiles image in tensorboard (use SCATTERGRAM for larger numbers of tiles. ANNOTATED_TILES looks a little nicer but it generates each tile as a separate subplot which can be very slow, and also has a much lower upper limit on the number of tiles it can handle)
...
elif [[ ${DATASET} == "skin" ]]; then
  
  CANCER_TYPE="Skin_Cancer"
  CANCER_TYPE_LONG="Skin_Cancer"
  STAIN_NORM_TARGET="./7e13fe2a-3d6e-487f-900d-f5891d986aa2/TCGA-CG-4301-01A-01-TS1.4d30d6f5-c4e3-4e1b-aff2-4b30d56695ea.svs"   # <--THIS SLIDE IS ONLY PRESENT IN THE FULL STAD SET & THE TARGET_TILE_COORDS COORDINATES BELOW ARE FOR IT
  TARGET_TILE_COORDS="5000 5500"
