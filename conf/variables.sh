...
if [[ ${DATASET} == "stad" ]]; 
  ...
  if [[ ${INPUT_MODE} == "image" ]]; 
    then
      ...
      SUPERGRID_SIZE=6                                                    # for test mode only: enables 'super-patches' that combinine multiple batches into a grid [test_mode (only). Minimum/default value=1; maximum value depends in TILES_PER_IMAGE
      TILE_SIZE="128"                                                     # 
      TILES_PER_IMAGE=144                                                  # for MOODUS: max 100 (training) 13/6084/36 (test mode); for DREEDLE: max ___ (training) 45^2 (?????) (test mode)
      BATCH_SIZE=4
