...
if [[ ${DATASET} == "stad" ]]; 
  ...
  if [[ ${INPUT_MODE} == "image" ]]; 
    then
      N_SAMPLES=200                                                         # on MOODUS 233 valid samples for STAD but use 232 / image; on DREEDLE 229 valid samples for STAD (but set N_SAMPLES=228)
      ...
      SUPERGRID_SIZE=70                                                    # for test mode only: enables 'super-patches' that combinine multiple batches into a grid [test_mode (only). Minimum/default value=1; maximum value depends in TILES_PER_IMAGE
      TILE_SIZE="128"                                                     # 
      TILES_PER_IMAGE=53                                                 # Training mode only (automatically calculated as SUPERGRID_SIZE^2 * BATCH_SIZE for test_mode)
      BATCH_SIZE="144"
