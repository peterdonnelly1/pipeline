...
CLASS_COLOURS="darkorange         yellow        khaki               rosybrown                                  deepskyblue                      tomato                                      gold         white"

if [[ ${DATASET} == "stad" ]]; 
  then
  if [[ ${INPUT_MODE} == "image" ]]; 
    then
      N_SAMPLES=228                                                       # on MOODUS 233 valid samples for STAD but use 232 / image; on DREEDLE 229 valid samples for STAD (but set N_SAMPLES=228)
      ...
      STAIN_NORMALIZATION="NONE reinhard spcn"                                          # options are NONE, reinhard, spcn  (specifies the type of stain colour normalization to be performed)
