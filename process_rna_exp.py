...
def main(args):
  ...
  if (DEBUG>0):
    ...
    print ( "PROCESS_RNA_SEQ:        INFO: args.rna_file_reduced_suffix   = {:}{:}{:}".format( BB, rna_file_reduced_suffix , RESET ),  flush=True )
  ...
  if (DEBUG>0):
    print ( "PROCESS_RNA_SEQ:        INFO: will look recursively under  {:}'{:}'{:} for files that match this pattern: {:}{:}{:}".format( BB, data_dir, RESET, BB, rna_file_reduced_suffix, RESET ),  flush=True ) 
  ...
  for root, __, files in walker:
    for f in files:
      ...
      if fnmatch.fnmatch( f, pattern  ):
        ...
        rna_expression_column = pd.read_csv(current_file, sep=',', usecols=[2])
