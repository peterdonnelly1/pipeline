...
--input_mode ${INPUT_MODE} --use_tiler 'external' --just_profile ${JUST_PROFILE}  --just_test ${JUST_TEST} --skip_preprocessing 'False' --skip_generation 'False' --dataset ${DATASET} --data_dir ${DATA_DIR} \
...
--class_numpy_file_name ${CLASS_NUMPY_FILENAME} --nn_mode ${NN_MODE} --use_same_seed ${USE_SAME_SEED} --nn_type ${NN_TYPE}  \
