  ...
  for root, dirs, files in walker:                                                                         # go through all the directories, but only tackle every my_thread'th directory
    ...
    for d in dirs:
      ...
      try:
        f = open(fq_name)
        if (DEBUG>0):
          print ( f"{SAVE_CURSOR}\033[{my_thread+55};110f  '{CYAN}SUFFICIENT_SLIDES_TILED{RESET}' flag seen - thread {MIKADO}{my_thread}{RESET} will now exit{RESET}{CLEAR_LINE}{RESTORE_CURSOR}", flush=True ) 
        sys.exit(0)
      except Exception:
        pass
