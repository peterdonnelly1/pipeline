...
class analyse_dataDataset( Dataset ):
    ...
    def __init__(self, cfg, args):
        ...
        print( f"A_D_DATASET:         INFO:     loading dataset from {MAGENTA}{cfg.ROOT_DIR }/train.pth{RESET}" )

        print( f"{ORANGE}A_D_DATASET:         INFO:     args.nn_mode = {MAGENTA}{args.nn_mode}{RESET}" )        
        ...
        if input_mode=='image':
          ...
          if args.nn_mode=='pre_compress':
            print( f"{ORANGE}A_D_DATASET:            INFO:  CAUTION! 'pre_compress' mode is set{RESET}" )            
          ...
          print ( f"{RED}A_D_DATASET:         FATAL:    unknown data mode \033[1m'{CYAN}{input_mode}{RESET}{RED} ... quitting{RESET}" )
          sys.exit(0)

        self.tissues    = data['tissues']                                                                  # self.tissues contains true labels for ALL the samples

        print( "A_D_DATASET:         INFO:     \033[3mdataset loaded\033[m" )
        ...
        if input_mode=='rna':
          if DEBUG>99:
            print ( f"A_D_DATASET:         INFO:     data['genes'][0] shape    = {CYAN}{data['genes'][0].cpu().numpy().shape }{RESET}"  )
          if DEBUG>999:
              np.set_printoptions(formatter={'float': lambda x: "{:>10.2f}".format(x)})
              print ( f"A_D_DATASET:         INFO:     data['genes'][0]          = \n{CYAN}{data['genes'][0:5].cpu().numpy()}{RESET}" )
                

        if DEBUG>99:
          print ( "A_D_DATASET:         INFO:     self.images shape          = \033[35;1m{:}\033[m".format( self.images.size() ) )
          if DEBUG>9:
            print ( "A_D_DATASET:         INFO:     self.images type           = {:}"  .format( type(self.images)    ) )
            print ( "A_D_DATASET:         INFO:     self.images                = \n{:}".format(  self.images[0]      ) )

        if DEBUG>9:
          print ( f"A_D_DATASET:         INFO:     self.tissues shape         = {CYAN}{self.tissues.size()}{RESET}          ")

        if DEBUG>0:
          np.set_printoptions(formatter={'int': lambda x: "{:>2d}".format(x)})
          print ( f"A_D_DATASET:         INFO:     self.tissues               = "     )
        ...
        if DEBUG>9999:
          print( "A_D_DATASET:         INFO:        __init__(): self.classes        = \n\033[35;1m{:}\033[m".format(    self.classes      ) )
        ...
        if DEBUG>1:
          print( "A_D_DATASET:         INFO:        __init__(): input_size           = \033[35;1m{:}\033[m".format  (   input_size        ) )
          print( "A_D_DATASET:         INFO:        __init__(): input_dimensions     = \033[35;1m{:}\033[m".format  (  input_dimensions   ) )
          print( "A_D_DATASET:         INFO:        __init__(): InputModeIsRna       = \033[35;1m{:}\033[m".format  (   InputModeIsRna    ) )
        if DEBUG>999:
          print( "A_D_DATASET:         INFO:        __init__(): self.tissues        = \n\033[35;1m{:}\033[m".format(    self.tissues     ) )

        labels_length         =  len(self.labels)

        if DEBUG>99:
          print( "A_D_DATASET:         INFO:        __init__(): labels_length         = \033[36;1m{:}\033[m".format (    labels_length        ) )
...
# ------------------------------------------------------------------------------
    ...
    def __len__(self):
        ...
        """
        if DEBUG>99:
          print( "A_D_DATASET:         INFO:     at __len__, and number of tiles in dataset = \033[36;1m{:}\033[m".format( len(self.images)))
...
# ------------------------------------------------------------------------------
    ...
    def __getitem__(self, i ):
        ...
        if DEBUG>9:
          print ( f"A_D_DATASET:         INFO:        __getitem__() ----------------------------------------------------------------- i                 = {i}" )
          print ( f"A_D_DATASET:         INFO:        __getitem__() ----------------------------------------------------------------- self.genes.dim()  = {self.genes.dim()}" )
...
# ------------------------------------------------------------------------------
    ...
    def sample_ith_image(self, i):
        ...
        print ( "A_D_DATASET:        hello from here in line 83 or so in sample_ith_image generate.py" )
