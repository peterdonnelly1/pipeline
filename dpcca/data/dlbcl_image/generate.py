...
def generate( args, class_names, n_samples, batch_size, highest_class_number, multimode_case_count, unimode_case_matched_count, unimode_case_unmatched_count, unimode_case____image_count, 
  ...
  if  (input_mode=='rna') & ( args.skip_generation=='True' ) & ( args.cases == 'ALL_ELIGIBLE_CASES') :    
    ...
    if DEBUG>0:
      print( f"{BOLD}{ORANGE}GENERATE:       INFO:  {CYAN}-g{RESET}{BOLD}{ORANGE} flag ({CYAN}SKIP_GENERATION{RESET}{BOLD}{ORANGE}) is set so loading pre-existing pytorch dataset (which MUST exist): {RESET}{MAGENTA}data/{args.nn_mode}/dataset_rna_train.pth{RESET}",  flush=True )
    try:
      data       = torch.load( fqn )
      genes      = data['genes']
