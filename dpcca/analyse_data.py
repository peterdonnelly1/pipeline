...
#      writer = SummaryWriter(comment=f' {dataset}; mode={input_mode}; NN={nn_type}; opt={nn_optimizer}; n_samps={n_samples}; n_t={n_tiles}; t_sz={tile_size}; rnd={rand_tiles}; tot_tiles={n_tiles * n_samples}; n_epochs={n_epochs}; bat={batch_size}; stain={stain_norm};  uniques>{min_uniques}; grey>{greyness}; sd<{min_tile_sd}; lr={lr}; lbl_swp={label_swap_perunit*100}%; greyscale={make_grey_perunit*100}% jit={jitter}%' )
   ...
   #pd.set_option( 'precision',               1 )
    ...
    if a_d_use_cupy=='True':
      ...
      if DEBUG>0:    
        print( f"ANALYSEDATA:        INFO:        {PINK}normalized df_cpy.shape        = {MIKADO}{df_cpy.shape}{RESET}", flush=True ) 
      if DEBUG>99:        
      ...
      if DEBUG>0:          
        print( f"ANALYSEDATA:        INFO:        {PURPLE}df_cpy.shape                   = {MIKADO}{df_cpy.shape}{RESET}", flush=True )
      scale_down=10000000
      ...
      if DEBUG>9:
        print( f"ANALYSEDATA:        INFO:        {PURPLE}df_cpy.shape                   = {MIKADO}{df_cpy.shape}{RESET}", flush=True )
        print( f"ANALYSEDATA:        INFO:        {PURPLE}index_of_cols.shape            = {MIKADO}{index_of_cols.shape}{RESET}", flush=True ) 
      if DEBUG>99:
        print( f"ANALYSEDATA:        INFO:        {PURPLE}index_of_cols                    = {MIKADO}{index_col}{RESET}", flush=True )  
      df_cpy = cupy.vstack ( [ index_col, df_cpy ])
      if DEBUG>9:
        print( f"ANALYSEDATA:        INFO:        {PURPLE}stacked df_cpy.shape           = {MIKADO}{df_cpy.shape}{RESET}", flush=True ) 
      if DEBUG>99:        
        np.set_printoptions(formatter={'float': lambda x: "{:>13.8f}".format(x)})
        print( f"ANALYSEDATA:        INFO:        {PURPLE}stacked df_cpy (start)         = \n{MIKADO}{df_cpy[0:10,0:12],}{RESET}", flush=True )
      ...
      if do_gpu_correlation=='True':
        ...
        if DEBUG>0:
          np.set_printoptions(formatter={'float': lambda x: "{:>13.8f}".format(x)}) 
          print( f"ANALYSEDATA:        INFO:         {PINK}col_i.shape                   = {MIKADO}{col_i.shape}{RESET}", flush=True  ) 
        col_i_expanded =  np.expand_dims( col_i, axis=1)
        if DEBUG>0:
          print( f"ANALYSEDATA:        INFO:         {PINK}col_i_expanded.shape          = {MIKADO}{col_i_expanded.shape}{RESET}", flush=True  )
        zero = np.zeros(([1,1]))
        if DEBUG>0:
          print( f"ANALYSEDATA:        INFO:         {PINK}zero.shape                    = {MIKADO}{zero.shape}{RESET}", flush=True  )
        h_stacked = np.vstack(( [ zero, col_i_expanded] ))
        if DEBUG>0:
          print( f"ANALYSEDATA:        INFO:         {PINK}h_stacked.shape               = {MIKADO}{h_stacked.shape}{RESET}", flush=True  ) 
        index_of_rows= h_stacked                                                                           # use hstack to add an arbitrary value (0) to the start of col_i array, because corr_cpy now has an index row atop it
