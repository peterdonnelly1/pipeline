...
# AT SOME POINT, CHANGE THE NAME OF THIS FLAG TO "USING_AUTOECODER"
  ...
  if args.peer_noise_perunit>0.0:
    print( f"P_C_DATASET:    INFO: CAUTION! {RED}{BOLD}ADD PEER NOISE{RESET} IS ACTIVE!; EACH TILE WILL RECEIVE {MIKADO}{args.peer_noise_perunit * 100:3.0f}%{RESET} NOISE FROM ANOTHER RANDONLY SELECTED TILE{RESET}" )  
...
# MOD TO ADD PEER NOISE MOD TO ADD PEER NOISE MOD TO ADD PEER NOISE MOD TO ADD PEER NOISE MOD TO ADD PEER NOISE MOD TO ADD PEER NOISE MOD TO ADD PEER NOISE MOD TO ADD PEER NOISE MOD TO ADD PEER NOISE 
        ...
        if args.peer_noise_perunit != 0:
          x2 = add_peer_noise( x2, args.peer_noise_perunit  )                                       # add peer noise to this batch of images
...
# THIS IS WHERE THE EMBEDDINGS ARE SAVED TO FILE THIS IS WHERE THE EMBEDDINGS ARE SAVED TO FILE THIS IS WHERE THE EMBEDDINGS ARE SAVED TO FILE THIS IS WHERE THE EMBEDDINGS ARE SAVED TO FILE
          ...
          if args.just_test=='True':                                                                       # In test mode (only), the z2 are the reduced dimensionality features that we want to save for use with NN models
            ...
            if DEBUG>1:
              ...
              print( f"PRE_COMPRESS:   INFO:        sanity check: z2          .shape = {MIKADO}{z2.shape}{RESET}" )
