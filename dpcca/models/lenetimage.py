...
class LENETIMAGE(nn.Module):
    ...
    def __init__(self, cfg, nn_type, n_classes, n_genes, tile_size, latent_dim, em_iters=1 ):
        ...
        self.image_net  = cfg.get_image_net( nn_type, n_classes, n_genes, tile_size )      #          get_image_net will return e.g. VGG11(self)   so model.get_image_net( nn_type ) = model.VGG11       (for example)
        self.genes_net  = cfg.get_genes_net(          n_classes, n_genes            )      #          get_genes_net will return e.g. DENSE(self)   so model.get_genes_net()          = model.DENSE
