"""=============================================================================
LeNet image analysis. For references in comments:
...
class LNETIMG(nn.Module):
    ...
    def __init__(self, latent_dim, dims, max_iters, debug=False):
        ...
        print( "LNETIMJ:        INFO:     at \033[35;1m__init__\033[m, parameters are: latent_dim=\033[35;1m{:}\033[m, dims=\033[35;1m{:}\033[m, max_iters=\033[35;1m{:}\033[m".format( latent_dim, dims, max_iters ) )
...
# ------------------------------------------------------------------------------
    ...
    def __repr__(self):
        """Compute a printable representation of a LNETIMJ object.
...
# ------------------------------------------------------------------------------
    ...
    def forward(self, y):
        ...
        """
        if DEBUG>9:
          print ( "LNETIMJ:          INFO:               forward(): type(y) = {:}".format( type(y)) )
          print ( "LNETIMJ:          INFO:               forward(): parameter y = observations of shape (n_features, n_samples) = {:}".format(y.size()) )
          
                  
        k = 3 * self.latent_dim

        if DEBUG>9:
          print ( "LNETIMJ:          INFO:               forward(): parameter self.latent_dim = {:}".format(self.latent_dim) )
          
        Lambda, Psi_diag = self.em_bishop(y)

        if DEBUG>9:
          print ( "LNETIMJ:          INFO:               forward(): parameter Lambda = {:}".format(Lambda) )
          print ( "LNETIMJ:          INFO:               forward(): parameter Psi_diag = {:}".format(Psi_diag) )
...
# ------------------------------------------------------------------------------
    ...
    def em_bishop(self, y):
        ...
        if DEBUG>9:
          print ( "LNETIMJ:          INFO:               forward(): shape of parameter Lambda   = {:}".format(Lambda.size()) )
          print ( "LNETIMJ:          INFO:               forward(): shape of parameter Psi_diag = {:}".format(Psi_diag.size()) )
        ...
        for _ in range(self.max_iters):
            ...
            if DEBUG>9:
              print ( "LNETIMJ:          INFO:                 forward(): for loop: self.max_iters              = {:}".format(self.max_iters) )
              print ( "LNETIMJ:          INFO:                 forward(): for loop: shape of parameter Psi_diag = {:}".format(Psi_diag.size()) )
              print ( "LNETIMJ:          INFO:                 forward(): for loop: shape of parameter Psi_diag = {:}".format(Psi_diag.size()) )
        ...
        if DEBUG>0:
          print ( "\0337\r\033[137C\033[38;2;140;140;140mem_bishop(): negative log likelihoods = \033[38;2;180;180;0m{}\033[m\u001b[0K\0338".format(nlls), end='' )
...
# ------------------------------------------------------------------------------
    ...
    def em_bishop_step(self, y, Lambda, Psi_diag):
        ...
        if DEBUG>9:
          print ( "LNETIMJ:          INFO:                   em_bishop_step(): shape of parameter Psi_diag  = {:}".format( Psi_diag.size() ) )
          print ( "LNETIMJ:          INFO:                   em_bishop_step(): shape of parameter Lambda    = {:}".format( Lambda.size()   ) ) 
          print ( "LNETIMJ:          INFO:                   em_bishop_step(): shape of parameter PLL_inv   = {:}".format( PLL_inv.size()  ) )
        ...
        if DEBUG>9:
          print ( "LNETIMJ:          INFO:                   em_bishop_step(): shape of parameter Ez   = {:}".format(Ez.size() ) )

        Ezz = self.E_zzT_given_y(Lambda, PLL_inv, y, k)

        if DEBUG>9:
          print ( "LNETIMJ:          INFO:                   em_bishop_step(): shape of parameter Ezz  = {:}".format(Ezz.size() ) )
...
# ------------------------------------------------------------------------------
    ...
    def E_z_given_y(self, L, PLL_inv, y):
        ...
        if DEBUG>9:
          print ( "LNETIMJ:          INFO:                     E_z_given_y(): shape of parameter PLL_inv    = {:}".format(PLL_inv.size() ) )

        assert len(PLL_inv.shape) == 2
        beta = L.t() @ PLL_inv

        if DEBUG>9:
          print ( "LNETIMJ:          INFO:                     E_z_given_y(): shape of parameter beta       = {:}".format(beta.size()     ) )
          print ( "LNETIMJ:          INFO:                     E_z_given_y(): shape of parameter L.t()      = {:}".format((L.t()).size() ) )
...
# ------------------------------------------------------------------------------
    ...
    def tile_params(self):
        """Tile parameters so we can use factor analysis updates for LNETIMJ.
...
# ------------------------------------------------------------------------------
    ...
    def parameters_(self, view=None):
        """Generates an iterator of parameters for the LNETIMJ instance.
