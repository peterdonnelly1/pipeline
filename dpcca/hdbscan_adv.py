...
try:
  ...
  if DEBUG>0:
    print ( f"HDBSCAN:         INFO:  genes_new.size         = {MIKADO}{genes_new.size()}{RESET}"      ) 
  if DEBUG>0:   
...
if DEBUG>0:
  print( f"HDBSCAN:         INFO:    image file shape {MIKADO}{x.shape}{RESET}" )
  print( f"HDBSCAN:         INFO:    label file shape {MIKADO}{y.shape}{RESET}" )  
  # ~ print( f"HDBSCAN:         INFO:    image file {MAGENTA}{img_file}{RESET}     contains {MIKADO}{x.shape[0]:,}{RESET} samples, each with {MIKADO}{x.shape[1]:,}{RESET} features:", flush=True)
  # ~ print( f"HDBSCAN:         INFO:    label file {MAGENTA}{img_labels_file}{RESET} contains {MIKADO}{x.shape[0]:,}{RESET} labels", flush=True)
...
if DEBUG>0:
  print( f"HDBSCAN:         INFO:  after splitting:" )
  print( f"HDBSCAN:         INFO:    Training set comprises {MIKADO}{training_examples}{RESET} samples" )
  print( f"HDBSCAN:         INFO:    Test     set comprises {MIKADO}{test_examples}{RESET}     samples" )
...
if DEBUG>0:
  print( f"HDBSCAN:         INFO:  about to configure {CYAN}TSNE{RESET} with: n_iter={MIKADO}{n_iter}{RESET}, perplexity={MIKADO}{perplexity}{RESET}, momentum={MIKADO}{momentum}{RESET},  metric='{CYAN}{metric}{RESET}', n_jobs={MIKADO}{n_jobs}{RESET}, random_state={MIKADO}{random_state}{RESET}", flush=True )
...
if DEBUG>0:
  print( f"HDBSCAN:         INFO:  about to run {CYAN}tsne.fit{RESET}", flush=True )
  
embedding_train = tsne.fit(x_train)

if DEBUG>0:
  print( f"HDBSCAN:         INFO:  finished {CYAN}tsne.fit{RESET}", flush=True )
  print( f"HDBSCAN:         INFO:  {CYAN}embedding_train.shape{RESET} ={MIKADO}{embedding_train.shape}{RESET}", flush=True )
  print( f"HDBSCAN:         INFO:  {CYAN}y_train.shape{RESET}         ={MIKADO}{y_train.shape}{RESET}",         flush=True )
  print( f"HDBSCAN:         INFO:  {CYAN}embedding_train{RESET}       =\n{MIKADO}{embedding_train}{RESET}",     flush=True )
