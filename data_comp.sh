...
if [[ ${SKIP_GENERATION} == "False" ]]
  ...
  echo ""
  echo "DELETING ANY EXISTING PICKLE FILES FROM ${BASE_DIR}/classi/modes/analyse_data/"
  
  rm ${BASE_DIR}/${APPLICATION_DIR}/modes/analyse_data/genes.pickle
  rm ${BASE_DIR}/${APPLICATION_DIR}/modes/analyse_data/genes_cupy.pickle.npy
...
#~ ./do_all.sh  -d ${DATASET}  -n analyse_data -i rna -8 ${LOW_EXPRESSION_THRESHOLD} -9 ${CUTOFF_PERCENTILE} -K ${HIGH_CORRELATION_THRESHOLD} \
