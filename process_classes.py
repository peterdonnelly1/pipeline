...
def main(args):
  ...
  if cases_processed_count==0:
    print ( f"{BOLD_RED}PROCESS_CLASSES:        FATAL:  no cases at all were successfully processed{RESET}" )
    print ( f"{BOLD_RED}PROCESS_CLASSES:        FATAL:  possible cause: perhaps you changed to a different cancer type (now '{CYAN}{args.dataset}{RESET}{BOLD_RED}') but did not regenerate the dataset?{RESET}" )
    print ( f"{BOLD_RED}PROCESS_CLASSES:        FATAL:                  if so, use the {CYAN}-r {RESET}{BOLD_RED}option ('{CYAN}REGEN{RESET}{BOLD_RED}') to force the dataset to be regenerated into the working directory{RESET}" )
    print ( f"{BOLD_RED}PROCESS_CLASSES:        FATAL:                  e.g. '{CYAN}./do_all.sh -d {dataset} -i {'image' if args.input_mode=='image' else 'rna'} ... {CHARTREUSE}-r True{RESET}{BOLD_RED}'{RESET}" )
    print(  f"{BOLD_RED}PROCESS_CLASSES:        FATAL: ... halting now{RESET}\n\n" )
    time.sleep(10)                                    
    sys.exit(0)   
  
  if len(all_classes)==0:
    print ( f"{BOLD_RED}PROCESS_CLASSES:        FATAL:  no classes (subtypes) cases at all were seen when processing the cases{RESET}" )
    print ( f"{BOLD_RED}PROCESS_CLASSES:        FATAL:  possible cause: perhaps you changed to a different cancer type but did not regenerate the dataset?{RESET}" )
    print ( f"{BOLD_RED}PROCESS_CLASSES:        FATAL:                  if so, use the {CYAN}-r {RESET}{BOLD_RED}option ('{CYAN}REGEN{RESET}{BOLD_RED}') to force the dataset to be regenerated into the working directory{RESET}" )
    print ( f"{BOLD_RED}PROCESS_CLASSES:        FATAL:                  e.g. '{CYAN}./do_all.sh -d {dataset} -i {'image' if args.input_mode=='image' else 'rna'} ... {CHARTREUSE}-r True{RESET}{BOLD_RED}'{RESET}" )
    print(  f"{BOLD_RED}PROCESS_CLASSES:        FATAL: ... halting now{RESET}\n\n" )
    time.sleep(10)                                    
...
if __name__ == '__main__':
  ...
  p.add_argument('--input_mode',              type=str                                                   )  
