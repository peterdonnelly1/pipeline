...
class ANALYSEDATA(nn.Module):
    ...
    def __init__(self, cfg, nn_type, n_classes, n_genes, nn_dense_dropout_1, nn_dense_dropout_2, tile_size, latent_dim, em_iters=1):
        ...
        if DEBUG>2:
          print ( "ANALYSEDATA:          INFO  \033[38;1mat top of DPCCJ before super call\033[m" )

        super(PRECOMPRESS, self).__init__()

        if DEBUG>2:
          print ( "ANALYSEDATA:          INFO  \033[38;1mafter super call\033[m" ) 
          print ( f"ANALYSEDATA:          INFO  latent_dim, cfg.IMG_EMBED_DIM, cfg.N_GENES = {latent_dim}, {cfg.IMG_EMBED_DIM}, {cfg.N_GENES}" )
        ...
        if DEBUG>2:
          print ( "ANALYSEDATA:          INFO  \033[38;1mabout to call PCCJ()\033[m" )
...
# ------------------------------------------------------------------------------
    ...
    def encode(self, x):  ## NOT USED
        ...
        if DEBUG>0:
          print ( "ANALYSEDATA:          INFO:                encode(): y.shape [encoded version of x (image)] = {:}".format( y.shape ) )
                 
        if DEBUG>9:
          print ( "ANALYSEDATA:          INFO:                encode(): encoded tensor y =\n{:}\n".format( y ) )
