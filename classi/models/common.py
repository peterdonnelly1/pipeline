...
class COMMON( nn.Module ):
  ...
  def __init__(self, args, cfg, input_mode, nn_type_img, nn_type_rna, encoder_activation, n_classes, n_genes, hidden_layer_neurons, embedding_dimensions, nn_dense_dropout_1, nn_dense_dropout_2, tile_size, latent_dim, em_iters=1 ):
    ...
    if DEBUG>88:
      print ( "COMMON:     INFO:   latent_dim = {:}, cfg.IMG_EMBED_DIM = {:}, cfg.N_GENES = {:}".format( latent_dim, cfg.IMG_EMBED_DIM, cfg.N_GENES  ) )        
...
# ------------------------------------------------------------------------------
  ...
  def forward(self, x, gpu, encoder_activation):
    ...
    if DEBUG>99:
      print ( f"{CARRIBEAN_GREEN}COMMON:     INFO:           forward(): x1.type = {type(x1)}{RESET}", flush=True )
      print ( f"{BITTER_SWEET}COMMON:     INFO:           forward(): x2.type = {type(x2)}{RESET}", flush=True )
