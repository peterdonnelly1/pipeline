...
class PRECOMPRESS(nn.Module):
    ...
    def __init__(self, args, gpu, rank, cfg, input_mode, nn_type_img, nn_type_rna, encoder_activation, n_classes, n_genes, hidden_layer_neurons, embedding_dimensions, nn_dense_dropout_1, nn_dense_dropout_2, tile_size, latent_dim, em_iters=1):
        ...
        if ( input_mode=='image_rna' ) | ( input_mode=='image' ):  
          ...
          self.image_net  = cfg.get_image_net ( args, gpu, rank, input_mode, nn_type_img, encoder_activation, n_classes, n_genes, hidden_layer_neurons, embedding_dimensions, nn_dense_dropout_1, nn_dense_dropout_2, tile_size  )            # METHOD:   get_image_net will return DCGANAE128(self) so self.image_net = self.DCGANAE128
        ...
        if ( input_mode=='image_rna' ) | ( input_mode=='rna' ): 
          ...
          self.genes_net  = cfg.get_genes_net ( args, gpu, rank, input_mode, nn_type_rna, encoder_activation, n_classes, n_genes, hidden_layer_neurons, embedding_dimensions, nn_dense_dropout_1, nn_dense_dropout_2             )            # METHOD:   get_genes_net will return DENSE(self)   so model.genes_net = get_genes_net(...)
        ...
        if DEBUG>1:
          ...
          print( f"PRECOMPRESS:    INFO  args.embedding_dimensions  = {MIKADO}{args.embedding_dimensions}{RESET}"   )
          print( f"PRECOMPRESS:    INFO  embedding_dimensions       = {MIKADO}{embedding_dimensions}{RESET}"   )
        
        self.pcca = PCCA (
                           latent_dim = latent_dim,                                                            # OBJECT:   PCCA is a class, hence self.pcca = model.pcca
                           dims       = [cfg.IMG_EMBED_DIM, embedding_dimensions],
                           max_iters  = em_iters
                         )
...
# ------------------------------------------------------------------------------
    ...
    def forward( self, args, x, input_mode, gpu, encoder_activation ):
        ...
        if input_mode=='image':
          x2r = self.image_net.forward( args, x, gpu, input_mode, encoder_activation )

        if input_mode=='rna':
          x2r = self.genes_net.forward( args, x, gpu,  input_mode, encoder_activation )
...
# ------------------------------------------------------------------------------
    ...
    def encode( self, args, x, input_mode, gpu, encoder_activation ):
        ...
        if input_mode=='image':
          z = self.image_net.encode( args, x, input_mode, gpu, encoder_activation  )

        if input_mode=='rna':
          z = self.genes_net.encode( args, x, input_mode, gpu, encoder_activation   )
...
# ------------------------------------------------------------------------------
    ...
    def sample_x2_from_x1(self, x1):
        ...
        with torch.no_grad():
          ...
          y2 = torch.zeros(x1.shape[0], self.args.embedding_dimensions, device=device)                              # self is DPCCA object model (nn.Module) hence self.cfg.IMG_EMBED_DIM = 'model.cfg.IMG_EMBED_DIM'
