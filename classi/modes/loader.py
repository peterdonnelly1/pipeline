...
def get_data_loaders( args, gpu, cfg, world_size, rank, batch_size, n_samples, num_workers, pin_memory, pct_test, writer, directory=None) :
    ...
    if just_test!='True':
      ...
      if DEBUG>0:
        ...
        if args.input_mode == 'image':
          print( f"{CLEAR_LINE}LOADER:         INFO:                                                                 tiles per batch:   {MIKADO}{int(train_cases/n_tiles[0]):>6d}, {int(test_cases/n_tiles[0]):>5d}{RESET}" )
      ...
      if DEBUG>0:
        ...
        if args.input_mode == 'image':
          print( f"{CLEAR_LINE}LOADER:         INFO: (just_test)                                                                                      tiles per batch:   {MIKADO}{int(len(test_inds)/n_tiles[0]):>5d}{RESET}"                  , flush=True )
