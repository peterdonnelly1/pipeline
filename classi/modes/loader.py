    ...
    if DEBUG>0:
      if input_mode=='image':
        if just_test != 'True':
          ...
          percent_tiles_used      = 100*used_tiles/available_tiles
          percent_test_tiles_used = 100*used_test_tiles/test_cases 
          print( f"{ORANGE}CLASSI:         INFO:   available      tiles = {MIKADO}{train_cases:>5d}{RESET}{ORANGE} + {MIKADO}{test_cases:>4d}{RESET}{ORANGE} \r\033[60C = {MIKADO}{available_tiles:>5d}{RESET}{ORANGE}.  Tiles      that will be used{RESET}{ORANGE} = total batches ({MIKADO}{total_batches:>4d}{RESET}{ORANGE}) * batch_size ({MIKADO}{batch_size:>3d}{RESET}{ORANGE}) = {MIKADO}{used_tiles}{RESET}{ORANGE}. Hence {MIKADO}{used_tiles:>5d}{RESET}{ORANGE}/{MIKADO}{available_tiles:>5d}{RESET}{ORANGE} = {BRIGHT_GREEN if percent_tiles_used >97 else RED if percent_tiles_used <95 else MIKADO}{percent_tiles_used:.0f}%{RESET}{ORANGE} of the available      tiles will be used.                          Change {CYAN}BATCH_SIZE{RESET}{ORANGE} or {CYAN}PCT_TEST{RESET}{ORANGE} if this is unacceptable{RESET}")
          print( f"{ORANGE}CLASSI:         INFO:   available test tiles \r\033[60C = {MIKADO}{test_cases:>5d}{RESET}{ORANGE}.  Test tiles that will be used{RESET}{ORANGE} = test  batches ({MIKADO}{number_of_test_batches:>4d}{RESET}{ORANGE}) * batch_size ({MIKADO}{batch_size:>3d}{RESET}{ORANGE}) = {MIKADO}{used_test_tiles}{RESET}{ORANGE}.  Hence {MIKADO}{used_test_tiles:>5d}{RESET}{ORANGE}/{MIKADO}{test_cases:>5d}{RESET}{ORANGE} = {BRIGHT_GREEN if percent_test_tiles_used >98 else RED if percent_test_tiles_used <95 else MIKADO}{100*used_test_tiles/test_cases:.0f}%{RESET}{ORANGE} of the available test tiles will be used during per-epoch testing{RESET}{ORANGE}. Change {CYAN}BATCH_SIZE{RESET}{ORANGE} or {CYAN}PCT_TEST{RESET}{ORANGE} if this is unacceptable{RESET}")
          print( "\n" )
      else:
