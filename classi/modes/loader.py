    ...
    if DEBUG>0:
      if input_mode=='image':
        if just_test != 'True':
          ...
          print( f"{ORANGE}LOADER:         INFO:   available training tiles \r\033[60C = {MIKADO}{train_cases:>5d}{RESET}{ORANGE}.  Training tiles that will be used{RESET}{ORANGE} = training  batches ({MIKADO}{number_of_train_batches:>4d}{RESET}{ORANGE}) * batch_size ({MIKADO}{batch_size:>3d}{RESET}{ORANGE}) = {MIKADO}{used_train_tiles:>5d}{RESET}{ORANGE}.  Hence {MIKADO}{used_train_tiles:>5d}{RESET}{ORANGE}/{MIKADO}{train_cases:>5d}{RESET}{ORANGE} =~{BRIGHT_GREEN if percent_train_tiles_used >98 else RED if percent_train_tiles_used <95 else BOLD_ORANGE}{100*used_train_tiles/train_cases:>2.0f}%{RESET}{ORANGE} of the available training tiles will be used in           training{RESET}{ORANGE}. Change {CYAN}BATCH_SIZE{RESET}{ORANGE} &/or {CYAN}PCT_TEST{RESET}{ORANGE} if this is unacceptable{RESET}")
          print( f"{ORANGE}LOADER:         INFO:   available test     tiles \r\033[60C = {MIKADO}{ test_cases:>5d}{RESET}{ORANGE}.  Test     tiles that will be used{RESET}{ORANGE} = test      batches ({MIKADO}{ number_of_test_batches:>4d}{RESET}{ORANGE}) * batch_size ({MIKADO}{batch_size:>3d}{RESET}{ORANGE}) = {MIKADO}{ used_test_tiles:>5d}{RESET}{ORANGE}.  Hence {MIKADO}{used_test_tiles :>5d}{RESET}{ORANGE}/{MIKADO}{ test_cases:>5d}{RESET}{ORANGE} =~{BRIGHT_GREEN if percent_test_tiles_used  >98 else RED if percent_test_tiles_used  <95 else BOLD_ORANGE}{100*used_test_tiles / test_cases:>2.0f}%{RESET}{ORANGE} of the available test     tiles will be used in per-epoch testing{RESET}{ORANGE}.  Change {CYAN}BATCH_SIZE{RESET}{ORANGE} &/or {CYAN}PCT_TEST{RESET}{ORANGE} if this is unacceptable{RESET}")
          print( "\n" )
      else:
        used_samples   = total_batches*batch_size
        unused_samples = n_samples - used_samples 
        percent_unused = 100*unused_samples/n_samples
        print( f"{ORANGE}LOADER:         INFO:   {CYAN}N_SAMPLES{RESET}{ORANGE} is {MIKADO}{n_samples}{RESET}{ORANGE}, total batches = {MIKADO}{total_batches}{RESET}{ORANGE} and {CYAN}BATCH_SIZE{RESET}{ORANGE} = {MIKADO}{batch_size}{RESET}{ORANGE}. This means {MIKADO}{used_samples}{RESET}{ORANGE} out of the {MIKADO}{n_samples}{RESET}{ORANGE} available samples ({MIKADO}{100-percent_unused:.0f}%{RESET}{ORANGE}) are being used in this training run{RESET}{ORANGE}. Change {CYAN}BATCH_SIZE{RESET}{ORANGE} and {CYAN}PCT_TEST{RESET}{ORANGE} if this figure is unacceptable{RESET}")
        print( "\n" )
