...
def generate_rna_dataset ( args, class_names, target, cases_required, highest_class_number, case_designation_flag, n_genes, low_expression_threshold, cutoff_percentile, gene_data_norm, gene_data_transform, use_autoencoder_output  ):
  ...
  if  ( args.just_test!='True' ):
    ...
    if len(np.unique(rna_labels_new)) != len(class_names):
      ...
      print ( f"{BOLD}{MAGENTA}GENERATE:       FATAL: it's also possible that, by chance, no representatives of one of the smaller classes (subtypes) made it into the training set. Re-running with option {CYAN}-v {RESET}{BOLD}{MAGENTA} set to {CYAN}True{RESET}{BOLD}{MAGENTA} tmay remedy this (might need to try multiple times) {RESET}{CLEAR_LINE}",      flush=True  )                                        
