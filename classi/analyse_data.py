...
samples={MIKADO}{args.n_samples[0]}{RESET}",
  ...
  threshold                   = args.low_expression_threshold
...
#      writer = SummaryWriter(comment=f' {dataset}; mode={input_mode}; NN={nn_type_rna}; opt={nn_optimizer}; n_samps={n_samples}; n_t={n_tiles}; t_sz={tile_size}; rnd={rand_tiles}; tot_tiles={n_tiles * n_samples}; n_epochs={n_epochs}; bat={batch_size}; stain={stain_norm};  uniques>{min_uniques}; grey>{greyness}; sd<{min_tile_sd}; lr={lr}; lbl_swp={label_swap_pct*100}%; greyscale={make_grey_pct*100}% jit={jitter}%' )
   ...
   #pd.set_option( 'precision',               1 )
    ...
    if use_cupy=='True':
      ...
      generate_file_name  = f'{base_dir}/{args.application_dir}/modes/analyse_data/genes_cupy.pickle.npy'
      if DEBUG>0:
    ...
    if use_cupy=='False':
      ...
      save_file_name  = f'{base_dir}/{args.application_dir}/modes/{mode}/genes_df_lo.pickle'                              # if it exists, just use it
      ...
      if os.path.isfile( save_file_name ):    
        ...
        generate_file_name  = f'{base_dir}/{args.application_dir}/modes/{mode}/genes.pickle'
...
if __name__ == '__main__':
  ...
  p.add_argument('--mode',                           type=str,   default='analyse_data')
