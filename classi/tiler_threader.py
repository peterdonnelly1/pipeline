...
def tiler_threader( args, flag, count, n_samples, n_tiles, top_up_factors, tile_size, batch_size, stain_norm, norm_method, zoom_out_mags, zoom_out_prob ):
  ...
  if ( ( just_test=='True')  & ( multimode!='image_rna' ) ):
    ...
    for n in range(0, num_cpus):
      task=executor.submit( tiler_scheduler, args, r_norm, flag, count, n_samples, n_tiles, top_up_factors, tile_size, batch_size, stain_norm, norm_method, zoom_out_mags, zoom_out_prob, n, num_cpus )
      tasks.append(task)
