...
has been set to {RESET}{BOLD_MIKADO} [ 'plane', 'car', 'bird', 'cat', 'deer', 'dog', 'frog', 'horse', 'ship', 'truck'  ]{RESET}{BOLD_CORAL}", flush=True)
  ...
  b  = "BALANCED"
  nb = "NOTBLNCD"
  job_descriptor = f"BATCH__{total_runs_in_job:03d}_{args.dataset.upper()}_{input_mode.upper():_<9s}_{b if args.make_balanced=='level_up' else nb}_{args.cases[0:20]:_<20s}"
...
{RESET}" ) 
        ...
        if ( ( already_tiled==True ) & ( ( stain_norm==last_stain_norm ) | (last_stain_norm=="NULL") ) & (n_tiles<=n_tiles_last ) & ( n_samples<=n_samples_last ) & ( tile_size_last==tile_size ) & ( must_tile==False )):
          ...
          if just_test=='True':
              ...
              if (  args.cases == 'UNIMODE_CASE' ):
                ...
                total_slides_counted_test, tiles_needed_per_subtype_test, total_tiles_required_test, top_up_factors_test  = determine_top_up_factors ( args, n_classes, class_names, n_tiles, flag )
                ...
                slides_tiled_count = tiler_threader( args, flag, total_slides_counted_test, tiles_needed_per_subtype_test, n_classes, n_samples, n_tiles, top_up_factors_test, tile_size, batch_size, stain_norm, norm_method, zoom_out_mags, zoom_out_prob  )
              ...
              if (  args.cases == 'ALL_ELIGIBLE_CASES' ):
                ...
                slides_tiled_count = tiler_threader( args, flag, slides_to_be_tiled, tiles_needed_per_subtype_test, n_classes, n_samples, n_tiles_max, top_up_factors_test, tile_size, batch_size, stain_norm, norm_method, zoom_out_mags, zoom_out_prob  )               # we tile the largest number of samples & tiles that is required for any run within the job
            ...
            if (  args.cases == 'ALL_ELIGIBLE_CASES' ):
              ...
              flag  = 'HAS_IMAGE'
              total_slides_counted_train, tiles_needed_per_subtype_train, total_tiles_required_train, top_up_factors_train  = determine_top_up_factors ( args, n_classes, class_names, n_tiles, flag )
                     
              if DEBUG>3:
                print( f"{SAVE_CURSOR}\r\033[{num_cpus+1}B{WHITE}CLASSI:         INFO: about to call tiler_threader with flag = {CYAN}{flag}{RESET}; slides_to_be_tiled = {MIKADO}{slides_to_be_tiled:3d}{RESET};   pct_test = {MIKADO}{pct_test:2.2f}{RESET};   n_samples_max = {MIKADO}{n_samples_max:3d}{RESET};   n_tiles_max = {MIKADO}{n_tiles_max}{RESET}{RESTORE_CURSOR}", flush=True )
              slides_tiled_count = tiler_threader( args, flag, slides_to_be_tiled, tiles_needed_per_subtype_train, n_classes, n_samples, n_tiles_max, top_up_factors_train, tile_size, batch_size, stain_norm, norm_method, zoom_out_mags, zoom_out_prob  )               # we tile the largest number of samples & tiles that is required for any run within the job
            ...
            if (  args.cases == 'UNIMODE_CASE' ):
              ...
              slides_tiled_count = tiler_threader( args, flag, total_slides_counted_train, tiles_needed_per_subtype_train, n_classes, n_samples, n_tiles_max, top_up_factors_train, tile_size, batch_size, stain_norm, norm_method, zoom_out_mags, zoom_out_prob  )               # we tile the largest number of samples & tiles that is required for any run within the job
              ...
              slides_tiled_count = tiler_threader( args, flag, total_slides_counted_test, tiles_needed_per_subtype_test, n_classes, n_samples, n_tiles_max, top_up_factors_train, tile_size, batch_size, stain_norm, norm_method, zoom_out_mags, zoom_out_prob  )               # we tile the largest number of samples & tiles that is required for any run within the job
    ...
    if pretrain=='True':                                                                                   # then load the last pretrained (as defined) model
      ...
      try:
        ...
        # ~ print ( f"{RED}CLASSI:         FATAL:  exception was {RED}'{e}'{RESET}", flush=True)    
        if args.input_mode == 'image':
          print ( f"{RED}CLASSI:         FATAL:    explanation 1: this is a test run. ({CYAN}JUST_TEST==TRUE{RESET}{RED} (shell) or {CYAN}--just_test=='True'{RESET}{BOLD_RED} (python user argument). Did you forget to train a model ?{RESET}", flush=True)
          ...
          print ( f"{RED}CLASSI:         FATAL:    explanation 3: the neural network type and parameters must be the same as was used in training, including '{CYAN}NN_TYPE_IMG{RESET}{RED}'{RESET}", flush=True)
        if args.input_mode == 'rna':
          ...
          print ( f"{RED}CLASSI:         FATAL:    explanation 2: the neural network type and parameters relating to network topology must be the same as was used in training, e.g. '{CYAN}NN_TYPE_RNA{RESET}{RED}' and '{CYAN}HIDDEN_LAYER_NEURONS{RESET}{RED}'{RESET}", flush=True)
    ...
    for i, ( batch_images, batch_genes, image_labels, rna_labels, batch_fnames ) in  enumerate( test_loader ):
        ...
        if loss_type!='mean_squared_error':                                                                # autoencoders don't produce predictions,so ignore
          ...
          if ( args.input_mode=='image' ) & ( args.just_test=='True' ):
            ...
            if args.scattergram=='True':
              if DEBUG>0:
                  print ( f"{CLEAR_LINE}CLASSI:         INFO:      test:           global_batch_count {DIM_WHITE}(super-patch number){RESET} = {global_batch_count+1:5d}  {DIM_WHITE}({((global_batch_count+1)/(args.supergrid_size**2)):04.2f}){RESET}" )
            ...
            if global_batch_count%(args.supergrid_size**2)==0:                                             # establish grid arrays on the FIRST batch for each grid
              ...
              if DEBUG>0:
                print ( f"{CLEAR_LINE}CLASSI:         INFO:      test:             supergrid_size                                                 = {BLEU}{args.supergrid_size}{RESET} hence image dimensions (tiles*tiles*batch_size = {args.supergrid_size}*{args.supergrid_size}*{batch_size}={image_tile_width}) will be {BLEU}{image_tile_width}x{image_tile_width}{RESET}{CLEAR_LINE}"  )
              if DEBUG>90:
              ...
              if global_batch_count%(args.supergrid_size**2)==(args.supergrid_size**2)-1:                                                      # if it is the last batch to go into this grid (super-patch)
                ...
                grid_best_tile_probs_by_class              = np.transpose (np.expand_dims( grid_p_full_softmax_matrix.max( axis=0 ), axis=1 ))         # this is where we sum the totals across all tiles
                grid_best_tile_probs_by_class += random.uniform( 0.0001, 0.001)
                grid_best_tile_probs_by_class[ grid_best_tile_probs_by_class!=np.max(grid_best_tile_probs_by_class)] = 0.                              # only interested in the subtype with the  highest probability
                ...
                best_tile_probabilities_matrix[index]      = grid_best_tile_probs_by_class
                ...
                if DEBUG>99:
                  np.set_printoptions(formatter={'float': lambda x:   "{:>6.2f}".format(x)})
                  print ( f"CLASSI:         INFO:      test:             grid_tile_probabs_totals_by_class{ASPARAGUS}{RESET}                              = \n{BOLD}{ASPARAGUS}{grid_tile_probabs_totals_by_class}{RESET}", flush=True  )
                  ...
                  print ( f"CLASSI:         INFO:      test:             best_tile_probabilities_matrix{ASPARAGUS}{RESET}                                 = \n{BOLD}{ASPARAGUS}{best_tile_probabilities_matrix}{RESET}", flush=True  )
                ...
                if args.multimode!='image_rna':
                  ...
                  if args.scattergram=='True':
                    ...
                    try:
                      ...
                      if DEBUG>0:
                        print ( f"{CLEAR_LINE}CLASSI:         INFO:      test:        fq_link                                    =                 {MIKADO}{fq_link}{RESET}" )
        ...
        if loss_type!='mean_squared_error':        
          ...
          if ( args.input_mode=='rna' ) & ( args.just_test=='True' ):
            ...
            if DEBUG>222:
              print ( f"CLASSI:         INFO:      test: p_full_softmax_matrix            = {BLEU}{p_full_softmax_matrix}{RESET}"         )                                    
            ...
            if DEBUG>222:
              print ( f"CLASSI:         INFO:      test: probabilities_matrix            = {BOLD_PURPLE}{probabilities_matrix}{RESET}"         ) 
      ...
      if epoch % 1 == 0:                                                                                   # every ... epochs, do an analysis of the test results and display same
        ...
        if DEBUG>222:
          if args.input_mode == 'image':
            ...
            print ( "CLASSI:         INFO:      test:        y2_hat                            = {:}".format( y2_hat_values                    ) )
        ...
        if DEBUG>222:
          if args.input_mode == 'image':
            print ( "CLASSI:         INFO:      test:        y1_hat                            = {:}".format( y1_hat_values                    ) )
          else:
            print ( "CLASSI:         INFO:      test:        y2_hat                            = {:}".format( y2_hat_values                    ) )
...
def is_supported_image_format (f ):
  ...
  if ( ( f.endswith( 'svs' )) | ( f.endswith( 'SVS' )) | ( f.endswith( "jpg" ) ) | ( f.endswith( "jpeg" ) ) | ( f.endswith( "JPG" ) ) | ( f.endswith( "JPEG" ) ) ):
    return True
  else:
    return False
...
def determine_top_up_factors ( args, n_classes, class_names, n_tiles, case_designation_flag ):
  ...
  if (args.make_balanced!='level_up') & (args.make_balanced!='level_down'):                                                                       # cater for the default case
    ...
    if args.make_balanced=='level_down':
      tiles_needed_per_subtype = tiles_needed_per_example * np.min(class_counts)
      total_tiles_required     = num_classes              * np.min(class_counts)
    ...
    if DEBUG>0:
      ...
      print( f"\033[{row+7};{col}f{CLEAR_LINE}{leader}tiles_needed_per_{BOLD}subtype{RESET}     = {colour}{tiles_needed_per_subtype}{RESET}",        flush=True  )
      print( f"\033[{row+8};{col}f{CLEAR_LINE}{leader}total_tiles_required{RESET}         =    {colour}{total_tiles_required:,}{RESET}",                        flush=True  )

  return total_slides_counted, tiles_needed_per_subtype, total_tiles_required, top_up_factors
...
# ------------------------------------------------------------------------------
def analyse_probs( y_hat, image_labels_values ):

    DEBUG     = args.debug_level_classify
    LOG_LEVEL = args.log_level

    # convert output probabilities to predicted class
    _, preds_tensor = torch.max( y_hat, axis=1 )

    if DEBUG>222:
      y_hat_numpy = (y_hat.cpu().data).numpy()
      print ( "CLASSI:         INFO:      analyse_probs():               y_hat_numpy.shape           = {:}".format( y_hat_numpy.shape    ) ) 
      print ( "CLASSI:         INFO:      analyse_probs():               y_hat_numpy                 = \n{:}".format( y_hat_numpy      ) ) 
...
#    preds = np.squeeze( preds_tensor.cpu().numpy() )
    ...
    if DEBUG>222:
      ...
      print ( f"CLASSI:         INFO:      analyse_probs():               preds.shape               = {BOLD_CORAL}{preds.shape}{RESET}"    )
      print ( f"CLASSI:         INFO:      analyse_probs():               preds                     = \n{BOLD_CORAL}{preds}"  )  
    ...
    if DEBUG>222:
      ...
      print ( "CLASSI:         INFO:      analyse_probs():         SECOND GROUP BELOW: y_hat_numpy.T"   )
      np.set_printoptions(formatter={'int':   lambda x: "\033[1m{:^10d}\033[m".format(x)    }    )
      print ( preds[0:22] )
      np.set_printoptions(formatter={'float': lambda x: "{0:10.4f}".format(x) }    )
      print (  np.transpose(y_hat_numpy[0:22,:])  )

    p_full_softmax_matrix = functional.softmax( y_hat, dim=1).cpu().numpy()
    ...
    p_highest  = np.array(  [ functional.softmax( el, dim=0)[i].item() for i, el in zip(preds, y_hat) ]   )
    ...
    if args.just_test=='True':
      if DEBUG>222:
        np.set_printoptions(formatter={'float': lambda x: "{0:10.4f}".format(x) }    )
        print ( f"CLASSI:         INFO:      analyse_probs():               p_2nd_highest.shape            = {AMETHYST}{(np.array(p_2nd_highest)).shape}{RESET}"    )
        print ( f"CLASSI:         INFO:      analyse_probs():               p_2nd_highest                  = \n{AMETHYST}{p_2nd_highest}"                                   ) 
    ...
    if DEBUG>222:
      np.set_printoptions(formatter={'float': lambda x: "{0:10.4f}".format(x) }    )
      print ( f"CLASSI:         INFO:      analyse_probs():               p_true_class.shape               = {BOLD_ASPARAGUS}{p_true_class.shape}{RESET}"    )
      print ( f"CLASSI:         INFO:      analyse_probs():               p_true_class                     = \n{BOLD_ASPARAGUS}{p_true_class}"  )  
