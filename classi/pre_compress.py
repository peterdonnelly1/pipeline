...
network={CHARTREUSE}{args.mode}{WHITE}, \
...
embedding_dimensions={CHARTREUSE}{args.embedding_dimensions}{WHITE}, \
...
########################
  ...
  if  ( ( input_mode == 'image' ) & ( mode == 'pre_compress' ) &  ( not ( 'AE' in nn_type_img[0] ) )):
    print( f"{RED}PRE_COMPRESS:   FATAL:  the network model must be an autoencoder if mode='{MIKADO}{mode}{RESET}{RED}' (you have nn_type_img='{MIKADO}{nn_type_img[0]}{RESET}{RED}', which is not an autoencoder) ... halting now{RESET}" )
    sys.exit(0)
    
  if  ( ( input_mode == 'rna'   ) & ( mode == 'pre_compress' ) &  ( not ( 'AE' in nn_type_rna[0] ) )):
    print( f"{RED}PRE_COMPRESS:   FATAL:  when {CYAN}NN_TYPE_RNA{RESET}{RED}='{MIKADO}{mode}{RESET}{RED}', the network model must be an autoencoder (you have {CYAN}NN_TYPE_RNA{RESET}{RED}={MIKADO}{nn_type_rna[0]}{RESET}{RED}, which is not an autoencoder){RESET}" )
...
"
  ...
  if DEBUG>0:
    if input_mode=='image':
      ...
      for lr, pct_test, n_samples, batch_size, n_tiles, tile_size, rand_tiles, nn_type_img, nn_type_rna, hidden_layer_neurons, embedding_dimensions, nn_dense_dropout_1, nn_dense_dropout_2, nn_optimizer, stain_norm, gene_data_norm, gene_data_transform, label_swap_pct, make_grey_pct, jitter in product(*param_values):    
...
\r\033[{start_column+6*offset}C{embedding_dimensions:<5d}\
...
# ~ \r\033[{start_column+10*offset+second_offset}C{embedding_dimensions:<5d}\
...
# ~ \r\033[{start_column+1*offset+second_offset}C{jitter:}{RESET}" )    
    ...
    if input_mode=='image':
      ...
      writer = SummaryWriter(comment=f' {dataset}; {input_mode}; {nn_type_rna}; ACT={encoder_activation}; HID={hidden_layer_neurons}; EMB={embedding_dimensions}; d1={nn_dense_dropout_1}; d2={nn_dense_dropout_2}; {nn_optimizer}; samples={n_samples}; genes={n_genes}; g_norm={gene_data_norm}; g_xform={gene_data_transform}; EPOCHS={n_epochs}; BATCH={batch_size}; lr={lr}')
...
# PREP FOR EMBEDDINGS ACCUMULATION PREP FOR EMBEDDINGS ACCUMULATION PREP FOR EMBEDDINGS ACCUMULATION PREP FOR EMBEDDINGS ACCUMULATION PREP FOR EMBEDDINGS ACCUMULATION PREP FOR EMBEDDINGS ACCUMULATION PREP FOR EMBEDDINGS ACCUMULATION 
    ...
    if just_test=='True':
      ...
      if DEBUG>12:
        ...
        print( f"PRE_COMPRESS:   INFO:        embedding_dimensions =  {MAGENTA}{embedding_dimensions}{RESET}", flush=True )
...
# BELOW IS WHERE THE EMBEDDINGS ARE ACCUMULATED THIS IS WHERE THE EMBEDDINGS ARE ACCUMULATED THIS IS WHERE THE EMBEDDINGS ARE ACCUMULATED THIS IS WHERE THE EMBEDDINGS ARE ACCUMULATED
          ...
          if args.just_test=='True':                                                                       # In test mode (only), the embeddings are the reduced dimensionality features that we want to save for use with NN models
            ...
            embeddings  = model.encode  ( args, x2, args.input_mode, gpu, encoder_activation )             
...
def segment_cases( pct_test ):
  ...
  if args.divide_cases=='True':
      ...
      if not (dir_path==args.data_dir): 
        ...
        try:
          ...
          try:
            ...
            f.close
            if DEBUG>22:
              print ( f"{PALE_GREEN}PRE_COMPRESS:      INFO:       segment_cases():  case \r\033[55C'{MAGENTA}{dir_path}{RESET}{PALE_GREEN}' \r\033[122C has been flagged with the UNIMODE_CASE____IMAGE  \r\033[204C (count= {MIKADO}{designated_not_a_multimode_case____image_count+1}{RESET})",  flush=True )
    ...
    #
    #        Strategy: res-designate an appropriate number of the 'UNIMODE_CASE____IMAGE' to be 'UNIMODE_CASE____IMAGE_TEST' (delete the first flag)
  

    cases_to_designate = int(pct_test * designated_not_a_multimode_case____image_count)
        
    if DEBUG>0:
      print ( f"{DULL_WHITE}PRE_COMPRESS:     INFO:     segment_cases():  about to randomly designate {ARYLIDE}UNIMODE_CASE____IMAGE{RESET}{DULL_WHITE} cases as reserved image test cases by placing the flag {ARYLIDE}UNIMODE_CASE____IMAGE_TEST{RESET}{DULL_WHITE} in their case directories",  flush=True )
    ...
    while True:
      ...
      for dir_path, dirs, files in os.walk( args.data_dir, topdown=True ):
        ...
        if DEBUG>55:  
          print( f"{DIM_WHITE}PRE_COMPRESS:     INFO:  now considering case {ARYLIDE}{os.path.basename(dir_path)}{RESET}{DIM_WHITE} \r\033[130C as a candidate UNIMODE_CASE____IMAGE_TEST case  " ) 
        ...
        if not (dir_path==args.data_dir):                                                                         # the top level directory (dataset) has be skipped because it only contains sub-directories, not data
          try:
            ...
            f = open( fqn, 'r' )                
            if DEBUG>66:
              print ( f"{PALE_GREEN}PRE_COMPRESS:      INFO:   case   \r\033[55C'{RESET}{AMETHYST}{dir_path}{RESET}{PALE_GREEN} \r\033[130C is a     {CYAN}UNIMODE_CASE____IMAGE{RESET}{PALE_GREEN} case{RESET}",  flush=True )
            ...
            if ( selector==22 ) & ( designated_not_a_multimode_case____image_test_count<cases_to_designate ):   # used 22 but it could be any number
              ...
              try:
                ...
                if DEBUG>66:
                  print ( f"{BLEU}PRE_COMPRESS:      INFO:      segment_cases():  case  {RESET}{CYAN}{dir_path}{RESET}{BLEU} \r\033[130C has been randomly designated as a UNIMODE_CASE____IMAGE_TEST case  \r\033[204C (count= {MIKADO}{designated_not_a_multimode_case____image_test_count}{BLEU}{RESET})",  flush=True )
              except Exception:
                print( f"{RED}PRE_COMPRESS:  FATAL:  either could not create '{CYAN}UNIMODE_CASE____IMAGE_TEST{RESET}' file or delete the '{CYAN}UNIMODE_CASE____IMAGE{RESET}' " )  
                time.sleep(10)
                sys.exit(0)              
          except Exception:
            if DEBUG>66:
              print ( f"{RED}PRE_COMPRESS:      INFO:   case \r\033[55C'{MAGENTA}{dir_path}{RESET}{PALE_RED} \r\033[130C is not a {CYAN}UNIMODE_CASE____IMAGE{RESET}{RED} case - - skipping{RESET}",  flush=True )
...
if __name__ == '__main__':
  ...
  if DEBUG>99:
    print ( f"{GOLD}args.embedding_dimensions{RESET} =           <----------------------------------------------------------------------    {YELLOW}{args.embedding_dimensions}{RESET}")
