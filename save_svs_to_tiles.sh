...
#    printf "  SAVE_SVS_TO_TILES.SH: INFO: slide file name        = "${SVS}"\n"
    ...
    python ${BASE_DIR}/save_svs_to_tiles.py "--slide_name="${SLIDE_FILE_NAME} "--greyness="${MINIMUM_PERMITTED_GREYSCALE_RANGE} "--file_dir="${dir} "--my_thread="${MY_THREAD_NUMBER} "--n_tiles="${TILES_PER_IMAGE}  "--rand_tiles="${RANDOM_TILES} "--tile_size="${TILE_SIZE} "--min_uniques="${MINIMUM_PERMITTED_UNIQUE_VALUES}  "--whiteness="${MAXIMUM_PERMITTED_WHITENESS} "--include_white_tiles="${INCLUDE_WHITE_TILES} "--colour_norm="${COLOUR_NORMALIZATION}
