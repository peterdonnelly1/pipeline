...
---
 Then get some data:
  ...
  eg: TCGA sarcoma dataset: (just RNA-Seq)
    fetch and pre-process TCGA kidney RNA-Seq data (takes about 60min):
      mkdir -p pipeline/source_data/kidn
...
---
 Then get some data:
  ...
  fetch and pre-process TCGA kidney RNA-Seq data (takes about 60min):
    python ./gdc-fetch.py --debug=9 --dataset kidn --case_filter="filters/TCGA-KIDN_case_filter" --file_filter="filters/GLOBAL_file_filter_UQ" --max_cases=5000 --max_files=10  --output_dir=source_data/kidn
